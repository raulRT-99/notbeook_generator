...
def preprocess_dataset(dataset, features, normalizer, negative_data):
    cells = []
    ...
    cells.extend(normalize(normalizer, negative_data))
    if negative_data:
...
def normalize(normalizer, negative_data):
    cells = []
    ...
    text_code += ("df_scaled = pd.DataFrame(rescaledX, columns=names[:8])\n"
                  "df_scaled['class'] = y\n\n")
    if negative_data:
