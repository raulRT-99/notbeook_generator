...
def onCreate(data, _):
    ...
    features = data.data.target_feature.get() if data.data.target_feature.get() else '***FEATURE COLUMNS***'
    ...
    if bool_output and not bool_dataset:
        output_folder = data.init_file.get().strip().parent()
    ...
    if describe:
        Notebook.resume_plots = data.resume_plots_cb.get()
    ...
    return create_notebook(_, output_folder, dataset, Notebook, data.enable_prediction.get(),
